...
import { ResourceDelimiter } from "./aws.api.enums.js";
...
export const arnSchema = z.object({
   ...
   * ```
   */
  resourceDelimiter: z.enum(ResourceDelimiter).optional(),
  /**
   * ```xml
...
export const serviceSchema = z.object({
   ...
   * ```
   */
  arnNamespace: z.string().regex(new RegExp("^[a-z0-9.\\-]{1,63}$")).optional(),
  /**
   * ```xml
   ...
   * ```
   */
  cloudFormationName: z.string().regex(new RegExp("^[A-Z][A-Za-z0-9]+$")).optional(),
  /**
   * ```xml
   ...
   * ```
   */
  cloudWatchNamespace: z.string().min(1).max(255).regex(new RegExp("^[^:].*$")).optional(),
});
